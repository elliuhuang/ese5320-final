...
void lzw(unsigned char *chunk, int chunk_len, cl_short *compressed, int *compressed_length, Node *tree) {

    if(chunk_len == 0) {
        throw "Zero sized chunk";
    }
    cl_short dictionary_index = 0;
    int chunk_index = 0;
    int count = 0;
    cl_short dictionary[4096][256];
    //Node tree[4096];
    ...
    //cl_short compressed[chunk_len];

    cl_short key = (cl_short)chunk[chunk_index];
    chunk_index++;


    while(chunk_index < chunk_len) {
        cl_short chunk_char = chunk[chunk_index];
        ...
        key = (cl_short)chunk[chunk_index];
        chunk_index++;
        count++;
    }
