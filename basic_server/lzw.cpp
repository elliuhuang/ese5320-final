...
void lzw(unsigned char *chunk, int chunk_len, unsigned char *compressed, int *compressed_length) {
    if (chunk_len == 0) {
        *compressed_length = 0;
        return;
    }
    ...
    DictEntry *dictionary = (DictEntry*)malloc(sizeof(DictEntry) * MAX_CHUNK_SIZE);
    for (int i = 0; i < 256; i++) {
    ...
    uint16_t *codes = (uint16_t*)malloc(sizeof(uint16_t) * MAX_CHUNK_SIZE);
    int code_count = 0;
